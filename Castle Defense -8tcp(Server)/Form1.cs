    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
          //  startfrm = new StartForm(this);
            ...
            mycamp = new basecamp(0, 50, 500, 1000, 1000, 10, 200, mybase);// x, y, 총골드량, 체력, 골드생성속도 ,방어력,감소할 체력, 이미지
            enemycamp = new basecamp(1300, 50, 500, 1000, 1000, 10, 200, enemybase);
        ...
        {
            new Thread(new ThreadStart(Server_Send)).Start();
            new Thread(new ThreadStart(Server_Recieve)).Start();
            start_timer();
        }
        IPEndPoint ipep = new IPEndPoint(IPAddress.Any, 9999);

        IPEndPoint irep = new IPEndPoint(IPAddress.Any, 7777);
        string msg = " ";
        ...
        public void Server_Recieve()
        {
            Socket server = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            server.Bind(irep);
            server.Listen(20);
            Socket client = server.Accept();
            MessageBox.Show("클라이언트와 연결되었습니다");
            String _data = " ";
            //수신
            while (true){
                
                Recieve(client, ref _data); //연결 Connection 
                if (_data == "unit1")
                {
                    recv_unit1_create();
                }
            ...
            this.Invoke(new MethodInvoker(
                delegate ()
                {
                    ...
                    for (int i = 0; i < unit_list.Count; i++)
                    {
                        ...
                        for (int c = 0; c < tower_list.Count(); c++)// 타워의 사정거리 내에 유닛 파악
                        {
                            if (tower_list[c].team != unit_list[i].team && CircleCrush(tower_list[c].x, tower_list[c].y, unit_list[i].x, unit_list[i].y, tower_list[0].range, unit_list[i].size_x))
                            {
                                double angle = getAngle(tower_list[c].x, tower_list[0].y, unit_list[i].x, unit_list[i].y);
                                tower_list[c].arrowx = tower_list[c].x + (int)((float)Math.Cos(angle * Math.PI / 180) * 40);
                                tower_list[c].arrowy = tower_list[c].y + (int)((float)Math.Sin(angle * Math.PI / 180) * 40);
                        ...
                        {
                            if (bullet_list[c].team != unit_list[i].team && CircleCrush(bullet_list[c].x, bullet_list[c].y, unit_list[i].x, unit_list[i].y, bullet_list[0].size, unit_list[i].size_x+5 ))
                            {
        ...
        public void EnemyUnit_Create(object sender, EventArgs e)
        {
            unit_list.Add(new unit(enemycamp.x, 2, 30, 10, 1, 50, 10, 25, 2, 30, 20, 40,true));
            Invalidate();
        }
        ...
        public void recv_unit1_create()
        {
            unit_list.Add(new unit(enemycamp.x, 2, 30, 5, 1, 50, 10, 25, 2, 30, 20, 40, true));
            //unit_list.Add(new unit(enemycamp.x, 2, 30, 10, 1, 50, 10, 25, 2, 30, 20, 40, true));
            Invalidate();
        }
        public void recv_unit2_create()
        {
            unit_list.Add(new unit(enemycamp.x, 2, 30, 10, 1, 100, 30, 25, 2, 30, 20, 40, true));
           // unit_list.Add(new unit(enemycamp.x , 2, 30, 10, 1, 100, 30, 25, 2, 30, 20, 40, true));
           // unit_list.Add(new unit(enemycamp.x , 2, 30, 10, 1, 100, 30, 25, 2, 30, 20, 40, true));//생성시x좌표,팀,체력,공격력,방어력,사거리,생성시 필요골드량,공격속도,이동속도,x크기,y크기
            Invalidate();
        }
